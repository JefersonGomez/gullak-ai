...
export default function StatsCards({ entries }) {
  ...
  const expensesChange = percentChange(currentPeriod.expenses, previousPeriod.expenses)
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      {cards.map(({ label, value, change, goodDirection, icon: Icon, color, bg }, i) => {
        const isPositiveChange = change >= 0
        const isGood = goodDirection === "up" ? isPositiveChange : !isPositiveChange
        const hasComparison = previousPeriod.sales > 0 || previousPeriod.expenses > 0
